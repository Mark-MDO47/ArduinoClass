...
Curiously, analog pins A4 and A5 are how most Arduinos connect to the internal I2C communication hardware. Seems strange to use analog pins for this purpose but that is the way it is. We will use these pins for I2C with the Arduino Nano.

I2C specifies that the pullup resistors be used (not shown in the above diagram). I have seen examples in the Arduino documentation with and without the pullup resistors. The behavior of I2C with and without external pullup resistors will depend on many factors such as the circuitry in the controller(s) and target(s), how many controllers and targets are in the circuit, how much wire is in the circuit, and other factors. In the case of the Arduino Nano and the DF2301QG in our project with a short connection and no other controllers or targets, it works pretty well without the pullup resistors. Nonetheless I think having the external pullup resistors in the circuit makes the communications better and more reliable.
